...
namespace OutwardVR
{
    public class VRCombat : MonoBehaviour
    {
        ...
        private float thresholdExceededModifier = 0.75f;
        ...
        private ArmIK rightHand;


        void Awake()
        {
            rightHand = base.transform.parent.parent.parent.GetComponent<ArmIK>();
        }
        ...
        void Update()
        {
            ...
            //if (!swingFired && Time.time - swingStart >= SWING_MAINTAINED_THRESHOLD)
            Character characterInstance = Camera.main.transform.root.GetComponent<Character>();
            ...
            if (isSwinging && !hasHit && Physics.Raycast(transform.parent.position, transform.right * -1, out hit, thresholdExceededModifier, LayerMask.GetMask("Hitbox")))
                hasHit = true;



            //Don't want blocking to activate mid swing
            if (swingVelocity < VELOCITY_THRESHOLD) {
                ...
                else if (isBlocking && Vector3.Dot(transform.right, characterInstance.transform.right) < 0.9f) {
                    // Unblock here
                    characterInstance.BlockInput(false);
                    isBlocking = false;
                    rightHand.SetDelay(0.7f);
                }
            }
