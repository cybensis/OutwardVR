...
namespace OutwardVR.camera
{
    public class CameraHandler
    {
        ...
        private static bool thirdPersonToggled = false;
        private static bool heightInitActivated = false;
        ...
        public class CharacterCamera_Update
        {
            ...
            public static bool Prefix(CharacterCamera __instance, Camera ___m_camera)
            {
                ...
                if (SteamVR_Actions._default.ClickRightJoystick.state || ControlsInput.m_playerInputManager[VRInstanceManager.currentPlayerId].GetButton(GetGameplayActionName(GameplayActions.LockToggle)))
                {
                    timeHeldFor += Time.deltaTime;
                    if (timeHeldFor > 1 && !thirdPersonToggled)
                    {
                        VRInstanceManager.ToggleThirdPerson();
                        thirdPersonToggled = true;
                    }
                }
                else if (ControlsInput.m_playerInputManager[VRInstanceManager.currentPlayerId].GetButton(GetGameplayActionName(GameplayActions.Stealth)))
                {
                    timeHeldFor += Time.deltaTime;
                    if (timeHeldFor > 1 && !heightInitActivated)
                    {
                        InitHeight();
                        heightInitActivated = true;
                    }
                }
                else
                    { 
                ...
                if (SteamVR_Actions._default.Start.stateDown)
                    InitHeight();
        ...
        private static void FixCamera(CharacterCamera cameraScript, Camera camera)
        {
            ...
            VRInstanceManager.ToggleMotionControls((VRInstanceManager.gamepadInUse) ? false : VRInstanceManager.firstPerson ? true : false);
        ...
        public class LocalCharacterControl_UpdateMovement
        {
                ...
                Transform ___m_horiControl, ref bool ___m_sprintFacing)
            {
                ...
                float xMovement = 0;
                float yMovement = 0;
                float xRot = SteamVR_Actions._default.RightJoystick.axis.x;

                if (!MiscPatches.characterUIInstance.IsMenuFocused && !MiscPatches.characterUIInstance.IsDialogueInProgress) {
                    if (VRInstanceManager.gamepadInUse)
                    {
                        xMovement = ControlsInput.MoveHorizontal(VRInstanceManager.currentPlayerId);
                        yMovement = ControlsInput.MoveVertical(VRInstanceManager.currentPlayerId);
                        xRot = ControlsInput.RotateCameraHorizontal(VRInstanceManager.currentPlayerId);
                    }
                    else {
                        yMovement = (SteamVR_Actions._default.LeftJoystick.GetAxis(SteamVR_Input_Sources.Any).y > 0.85f ? 1f : (SteamVR_Actions._default.LeftJoystick.GetAxis(SteamVR_Input_Sources.Any).y < -0.85f ? -1f : SteamVR_Actions._default.LeftJoystick.GetAxis(SteamVR_Input_Sources.Any).y));
                        xMovement = (SteamVR_Actions._default.LeftJoystick.GetAxis(SteamVR_Input_Sources.Any).x > 0.85f ? 1f : (SteamVR_Actions._default.LeftJoystick.GetAxis(SteamVR_Input_Sources.Any).x < -0.85f ? -1f : SteamVR_Actions._default.LeftJoystick.GetAxis(SteamVR_Input_Sources.Any).x));
                        ___m_modifMoveInput.x += xMovement * 4;
                    }
                }
                ...
                else
                {
                    ...
                    if (___m_inputMoveVector.y < -1)
                        ___m_inputMoveVector.y = yMovement * 4f;

                    // Keep forward sprint movement from exceeding 9 and keep everything else to  -4 to 4
                    if (m_char.Sprinting) { 
                        ___m_inputMoveVector.y = Mathf.Clamp(___m_inputMoveVector.y, -4, 9);
                    }
                    else {
                        ___m_inputMoveVector.y = Mathf.Clamp(___m_inputMoveVector.y, -4, 4);
                        ___m_inputMoveVector.x = Mathf.Clamp(___m_inputMoveVector.x, -4, 4);
